/*!
 * \file sbas_navigation_message.cc
 * \brief Implementation of a SBAS NAV Data message decoder 
 *
 * See RTCA DO-229 MOPS for SBAS
...
#include "sbas_navigation_message.h"
...
bool Sbas_Navigation_Message::read_navigation_bool(const std::bitset<SBAS_DATA_PAGE_BITS>& bits, const std::vector<std::pair<int32_t, int32_t>>& parameter) const
{
    bool value;

    if (static_cast<int>(bits[SBAS_DATA_PAGE_BITS - parameter[0].first]) == 1)
        {
            value = true;
        }
    else
...
uint64_t Sbas_Navigation_Message::read_navigation_unsigned(const std::bitset<SBAS_DATA_PAGE_BITS>& bits, const std::vector<std::pair<int32_t, int32_t>>& parameter) const
{
    uint64_t value = 0ULL;
            ...
            for (int32_t j = 0; j < parameter[i].second; j++)
                {
                    ...
                    if (static_cast<int>(bits[SBAS_DATA_PAGE_BITS - parameter[i].first - j]) == 1)
                        {
...
int64_t Sbas_Navigation_Message::read_navigation_signed(const std::bitset<SBAS_DATA_PAGE_BITS>& bits, const std::vector<std::pair<int32_t, int32_t>>& parameter) const
{
    int64_t value = 0LL;
    const int32_t num_of_slices = parameter.size();
...
void Sbas_Navigation_Message::decode_page(const std::bitset<SBAS_DATA_PAGE_BITS>& data_bits)
{
    int32_t page_type;
    bool alert_flag;
...
bool Sbas_Navigation_Message::have_new_ephemeris()  // Check if we have a new ephemeris stored in the galileo navigation class
{
...
Gps_CNAV_Ephemeris Sbas_Navigation_Message::get_ephemeris() const
{
    return ephemeris_record;
}


bool Sbas_Navigation_Message::have_new_iono()  // Check if we have a new iono data stored in the galileo navigation class
{
...
Gps_CNAV_Iono Sbas_Navigation_Message::get_iono() const
{
    return iono_record;
}


bool Sbas_Navigation_Message::have_new_utc_model()  // Check if we have a new iono data stored in the galileo navigation class
{
...
Gps_CNAV_Utc_Model Sbas_Navigation_Message::get_utc_model()
{
